...
const PopHoverCreate = ({ isCreating }: { isCreating: boolean}) => {
  return (
      <main className={`${isCreating ? 'opacity-100 w-full md:w-[435px] h-[290px] md:h-[238px] visible' : 'opacity-0 w-0 h-0 invisible'} z-10 transition-all ease-linear duration-200   bg-[rgba(255,255,255,0.05)] border-[2px] border-[rgba(255,255,255,0.08)] backdrop-blur-lg rounded-lg absolute bottom-[115px] lg:bottom-[170px] left-6`}>
        <section className='py-6 px-4 flex flex-col gap-y-6'>
              {list.map((item, index) => {
                  return (
                    <Link href={item.link} key={index} className='flex items-center justify-between'>
